# Tesla Mega Menu UI

This project is a **Tesla-inspired Mega Menu UI** built with **HTML, CSS, and JavaScript**.  
It features a responsive navbar with dropdown mega menus, images for each section, and right-aligned icons similar to Tesla.com.


## **Features**

- **Centered Navbar**: Main navigation is horizontally centered.
- **Right Corner Icons**: Includes `?` (Help), `🌐` (Language), and `👤` (User) icons.
- **Mega Menus**: Hover over each menu item to display a mega menu with images and links.
- **Hero Section**: Large background image with a heading and subtext.
- **Responsive Design**: Works on desktop and mobile screens with grid adjustment.
- **Smooth Hover Effects**: Mega menus appear with a slide/fade effect.
- **Side Links**: Some mega menus have additional side links for quick actions (e.g., Demo Drive, Compare).


# Technologies Used
HTML5
CSS3 (Flexbox, Grid, Responsive design)
JavaScript (DOM manipulation for mega menu interactions)


# Future Enhancements
Add slide + fade animation to make mega menus appear smoother.
Add clickable dropdown for mobile screens.
Integrate real links to Tesla pages or custom pages.
Add dark/light mode toggle.




